...
n(g) = nL(g) + sum(k$(ord(k) le (Kmax+1)), y(g,k)*(2**(ord(k)-1)));
 
eq1=sum(mt,w(mt));

m=(w('a') - w('b'));

eq2=sum(g,n(g) $ (info(g,'t') = 3)) - 6*w('m') - 10*w('b');

eq3=sum(g, (2-info(g,'v'))*n(g)) - 2*m;

eq4(j) = n(j)*(info(j,'v') - 1) + 2 - sum(g,n(g));


Tb = 204.359*log((sum(g,n(g)*info(g,'tb1k'))));

Tcrit = 181.128*log(sum(g,n(g)*info(g,'tc1k')));

Pcrit = 1.3705 + (sum(g,n(g)*info(g,'pc1k')) + 0.10022)**(-2);

h = (Tb/Tcrit)*((log(Pcrit)/1.01325)/(1 - Tb/Tcrit));

Gvap = 0.4835 + 0.4605*h;

kvap = ((h/Gvap) - (1 + Tb/Tcrit))/((3 + Tb/Tcrit)* (1 - Tb/Tcrit)**2);

vp_er = (-Gvap/(T('e')/Tcrit))*(1 - (T('e')/Tcrit)**2 + kvap*(3 + (T('e')/Tcrit))*power((1 - (T('e')/Tcrit)),3));

vp_cr = (-Gvap/(T('c')/Tcrit))*(1 - (T('c')/Tcrit)**2 + kvap*(3 + (T('c')/Tcrit))*power((1 - (T('c')/Tcrit)),3));

pi('vp_e') = exp(vp_er)*Pcrit;

pi('vp_c') = exp(vp_cr)*Pcrit;


h_vap298 = 6.829 + sum(g,n(g)*(info(g,'hv1k')));

pi('h_vap') = h_vap298*((1-(T('e')/Tcrit))/(1-(T('s')/Tcrit)))**0.375;


theta = (T('m') - 298)/700;

omega = 0.4085*(log((sum(g,n(g)*info(g, 'w1k'))) + 1.1507)**(1/0.5050));

cp_residual = R*(1.586 + 0.49/(1 - ((T('m'))/Tcrit)) + omega*(4.2775 + (6.3*(1 - (T('m')/Tcrit))**(1/3))/((T('m'))/Tcrit) + 0.4355/(1 - ((T('m'))/Tcrit))));

cp_ideal = ((sum(g,n(g)*info(g,'cpa1k')) - 19.7779) + ((sum(g,n(g)*info(g,'cpb1k'))) + 22.5981)*theta + ((sum(g,n(g)*info(g,'cpc1k'))) - 10.7983)*power(theta,2));
