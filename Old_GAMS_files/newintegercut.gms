...
Parameter
    ...
    yv(g,k,cc) store y values from previous solutions
...
Equation
    ...
    IntCut(cc) integer cut constraint
...
eqAux2.. sum(g,n(g)) =L= 10;

IntCut(cc).. sum((g,k),yv(g,k,cc)*y(g,k))-sum((g,k),(1-yv(g,k,cc))*y(g,k)) =L= sum((g,k),yv(g,k,cc)) - 1;
